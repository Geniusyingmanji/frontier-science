"""Oracle: Optimize Fabry-Perot laser cavity parameters. Oracle: round-trip gain/loss + mode stability (ABCD matrix propagation)."""
import numpy as np
...
def evaluate(design_cavity):
    try:
        params = np.asarray(design_cavity(N_PARAMS), dtype=float).ravel()
