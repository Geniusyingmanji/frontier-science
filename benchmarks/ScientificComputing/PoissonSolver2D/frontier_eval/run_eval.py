"""Black-box eval entrypoint for PoissonSolver2D."""
...
def main() -> int:
    ...
    try:
        ...
        solve_poisson = _load_callable(Path(args.candidate).resolve(), "solve_poisson")
        result = oracle.evaluate(solve_poisson)
    ...
    print(json.dumps({k: metrics.get(k) for k in ("combined_score","valid","rel_l2_error")}))
    return 0
