"""Oracle: Optimize rock bolt pattern for tunnel stability. Oracle: 2D plane-strain FEM with Mohr-Coulomb failure criterion."""
import numpy as np
...
def evaluate(design_support):
    try:
        params = np.asarray(design_support(N_PARAMS), dtype=float).ravel()
