"""Oracle: Fit LJ parameters to match RDF peaks. Oracle: MC simulation of 100 particles in NVT ensemble."""
import numpy as np
...
def evaluate(calibrate_forcefield):
    try:
        params = np.asarray(calibrate_forcefield(N_PARAMS), dtype=float).ravel()
